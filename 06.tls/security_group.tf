...
resource "aws_security_group_rule" "web_out_http" { # for yum update
  security_group_id = aws_security_group.web.id
  type              = "egress"
  protocol          = "tcp"
  from_port         = 80
...
resource "aws_security_group_rule" "web_out_https" { # for yum update
  security_group_id = aws_security_group.web.id
  type              = "egress"
  protocol          = "tcp"
  from_port         = 443
...
# 80 番の out が無いと動かない。おそらく TLS ハンドシェイクに使用しているものと思われる
...
resource "aws_security_group_rule" "alb_in_https" {
  ...
  to_port           = 443
  cidr_blocks       = ["0.0.0.0/0"]
  description       = "${var.project}-${var.environment}-alb-in-https-rule"
}